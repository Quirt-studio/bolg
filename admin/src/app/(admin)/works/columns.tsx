...
export function getColumns(actions: ColumnActions, t: TFunc): ColumnDef<Work>[] {
  return [
    {
      id: "select",
      header: ({ table }) => (
    ...
    },
    {
      ...
      header: ({ column }) => <DataTableColumnHeader column={column} title={t("works.column.title")} />,
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: ({ column }) => <DataTableColumnHeader column={column} title={t("works.column.category")} />,
    ...
    {
      accessorKey: "date",
      header: ({ column }) => <DataTableColumnHeader column={column} title={t("works.column.date")} />,
      cell: ({ row }) => <span className="text-muted-foreground">{format(new Date(row.original.date), "MMM dd, yyyy")}</span>,
    },
    {
      accessorKey: "featured",
      header: ({ column }) => <DataTableColumnHeader column={column} title={t("works.column.featured")} />,
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem>
              <Link href={`/works/${row.original.id}`} className="flex items-center w-full">
            ...
            <DropdownMenuItem onClick={() => actions.onDelete(String(row.original.id))} className="text-destructive focus:text-destructive">
              <Trash2 className="mr-2 h-4 w-4" />{t("works.delete")}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ),
